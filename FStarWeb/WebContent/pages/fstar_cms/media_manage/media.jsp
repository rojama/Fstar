...
<script type="text/javascript">  
        ...
        function urlgridAction(item)
        {
        	...
        	if (urlaction == "add"){
                ...
                $.ajax({
        			type:'POST',
        			url:'<%=serverUrl%>'+'&ACTION=saveUrl' + '&urldata='+urldata + '&media_id='+urlselect.media_id  ,
        			dataType:'json',
        			success:function(result)
        		    {
        				if (result.ERR == null){
        ...
        function toolbarAction(item)
        {
        	...
        	action = item.icon;
        	if (action == "add"){
        		$form.setData({media_id:'自动获取',media_name:'',media_describe:'',type_id:type_id,image:'',length:'0',total_series:'1',director:'',actor:'',score:'0.00'});
        		$form.setEnabled(['media_id','media_name','media_describe','type_id','image','length','total_series','director','actor','score'],true);
        		...
        		$form.setData(select);
        		$form.setEnabled(['media_id','media_name','media_describe','type_id','image','length','total_series','director','actor','score'],false);        		
        	}else if (action == "url"){
        		...
        		$dialog = $.ligerDialog.open({title: urlselect.media_name+item.text ,height: 500,width: 800,target: $("#urlgrid")});
        		$.ajax({
        			type:'POST',
